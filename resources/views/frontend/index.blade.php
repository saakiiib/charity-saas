    ...
    @foreach ($sections as $sectionName)
        @if ($sectionName === 'slider' && $sliders->count() > 0)
            ...
            <section class="hero hero-slider" data-slider>
                ...
                <div class="hero-dots" data-dots>
                    @foreach ($sliders as $slider)
                        <button class="{{ $loop->first ? 'is-active' : '' }}" data-go="{{ $loop->index }}"
            ...
            <section class="section section-alt">
                <div class="container">
                    <header class="section-head">
                        <h2>{{ $serviceContent?->short_title }}</h2>
                        <p>{{ $serviceContent?->long_title }}</p>
                    </header>
                    <div class="grid grid-2">
                        @foreach ($services as $index => $service)
                            <article class="card">
                                <span class="card-num">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }} ·
                                    {{ $service->title }}</span>
                                <h3>{{ $service->title }}</h3>
            ...
            <section class="section">
                <div class="container feature">
                    <div class="feature-media"
                        style="background-image:url('{{ asset('uploads/posts/' . $latestPost?->image) }}')">
                    </div>
                    ...
                    <div class="gallery">
                        @foreach ($galleries as $gallery)
                            <figure class="tile" data-lightbox
                                style="background-image:url('{{ asset('uploads/galleries/' . $gallery->image) }}')">
        ...
        @if ($sectionName === 'faq' && $faqs->count() > 0)
            ...
            <section class="section faq-section">
                <div class="container">
                    <header class="section-head reveal in" style="transition-delay: 0ms;"><span class="eyebrow">FAQ</span>
                        <h2>Frequently asked questions</h2>
                        <p>Everything you might want to know before you give, volunteer, or visit.</p>
                    </header>
                    <div class="faq" data-faq>
                        @foreach ($faqs as $faq)
                            <details class="faq-item reveal" {{ $loop->first ? 'open' : '' }}>
                                <summary><span>{{ $faq->question }}</span><i class="faq-ico" aria-hidden="true"></i>
                                </summary>
                                <div class="faq-body">
                                    <p>{!! $faq->answer !!}</p>
                                </div>
